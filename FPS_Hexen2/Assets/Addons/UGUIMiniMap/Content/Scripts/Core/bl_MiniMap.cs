...
public class bl_MiniMap : bl_MonoBehaviour
{
    ...
    void OnPlayerSpawn()
    {
        m_Target = bl_GameManager.Instance.LocalPlayer;
        if(compass != null) { compass.Target = m_Target.transform; }
        if (m_Mode == RenderMode.Mode3D) { ConfigureCamera3D(); }
    ...
    void CreateMapPlane(bool area)
    {
        ...
        if (m_Type != RenderType.Picture && !ShowAreaGrid) return;
    ...
    public void ConfigureCamera3D()
    {
        ...
        {
            cam = bl_GameManager.Instance.LocalPlayer.GetComponent<bl_PlayerSettings>().PlayerCamera;
        }
        if (cam == null)
        {
    ...
    void RotationControll()
    {
        ...
        if (DynamicRotation && m_MapType != MapType.Global)
        {
            ...
            if (SmoothRotation)
            {
                ...
                m_Transform.eulerAngles = Vector3.Lerp(m_Transform.eulerAngles, e, Time.deltaTime * LerpRotation);
            }
            else
            {
                m_Transform.eulerAngles = e;
    ...
    void MapSize()
    {
        ...
        MinimapCamera.orthographicSize = Mathf.Lerp(MinimapCamera.orthographicSize, DefaultHeight, delta * LerpHeight);
    }
