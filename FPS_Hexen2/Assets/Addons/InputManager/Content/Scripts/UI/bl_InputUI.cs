...
namespace MFPS.InputManager
{
    public class bl_InputUI : MonoBehaviour
    {
        ...
        public GameObject ConfirmChangeWindow;
        public Text ConfirmChangeText;
        public GameObject RevertButton, ApplyButton;

        public bool isFetchingKey { get; set; }  = false;
            ...
            if (Input.anyKeyDown)
            {
                foreach (KeyCode vKey in keyCodeList)
                {
                    if (Input.GetKeyDown(vKey))
                    {
                        ...
                        {
                            ConfirmChangeWindow.SetActive(true);
                            isFetchingKey = false;
                            break;
                        }
        ...
        public void ConfirmChange(bool confirm)
        {
            ...
            ConfirmChangeWindow.SetActive(false);
        }
        ...
        bool isKeyUsed(KeyCode key)
        {
            foreach (var data in bl_InputData.Instance.mappedInstance.ButtonMap)
            {
                if (data.PrimaryKey == key || data.AlternativeKey == key)
                {
                    ...
                    ConfirmChangeText.text = $"The key <b>{keyName}</b> is already used for <b>{data.Description}</b>, you want to change it anyway?";
        ...
        bool isAxisUsed(string axis)
        {
            foreach (var data in bl_InputData.Instance.mappedInstance.ButtonMap)
            {
                if (data.PrimaryAxis == axis)
                {
                    ...
                    ConfirmChangeText.text = $"The key <b>{axis}</b> is already used for <b>{data.Description}</b>, you want to change it anyway?";
                    pendingResetButton = data;
                    return true;
                }
            }
