...
public class bl_HeadBob : MonoBehaviour {
    ...
    [SerializeField]
    float headBobFrequencyRunning = 1.5f;				// the base speed of the head bobbing (in cycles per metre)
    [SerializeField]
    ...
    // private vars:
     bl_FirstPersonController character;						// a reference to the First Person Character component (on the parent gameobject)
    ...
    void FixedUpdate()
    {
        ...
        if (character.State == PlayerState.Running)
        {
