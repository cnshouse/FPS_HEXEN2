...
public class AddMapTutorial : TutorialWizard
{
    ...
    void DrawStarted()
    {
        DrawText("In this tutorial you will learn how to add a new map in order to play your own level within MFPS.");
        DownArrow();
        DrawText("First, what you need is of course a map. By map I mean a level design with all the art content models, prefabs, lights, sky, etc... placed in a way that looks like a battlefield.");
        DrawText("There are some basic requirements for the map that all Unity users should know that applies for all games, not just MFPS, If you are a beginner, lets review:\n \n" +
            "-<b>The map models/prefabs must have colliders</b>, except for the models that are used only as decoration. All models need colliders in order to keep the player from passing through them.\n \n" +
            "-<b>Lighting</b>, lighting is a important part of map, but affects the performance of game in a big way. There a tons of tutorials out there on how build a good scene lighting. \n \n" +
            "-<b>Optimized performance over good looking graphics</b> We all love good graphics in a game, but a poorly optimized level could kill your game. MFPS is well optimized with clean code." +
            " but more important than the code, at least in this case, is the graphic optimization. There is a good official Unity post for graphic optimization, check it out: ");
    ...
    void DrawSetup()
    {
        if (subStep == 0)
        {
            DrawText("Like I said before you need have your new map scene design ready, but not just as a prefab, you need have it placed in a Unity Scene (.scene)," +
                ...
                "Be sure to delete all other cameras in your map scene. They are not needed as cameras will be created by MFPS.");
        ...
        {
            DrawText("Ok, now after instanced the MFPS objects you will see in the Game View that there is a camera render from the top, that's the camera view that displays an aerial view when a player enters the room to select the team to join," +
        ...
        else if (subStep == 3)
        {
            DrawText("Now you need create some <b>Spawn Points</b> for each Team (Team 1, Team 2 and For FFA). These spawn points are not specifically a static position, they are a <b>Spherical Area</b> " +
                "where players can spawn, that's mean that the player will be instantiated in a random position inside of the radius of the area.\n \n<b><i>How create spawn points:</i></b>\nSimply create a empty game object in the scene" +
                " and add the script <b>bl_SpawnPoint.cs</b> to it and assign the area and the team. Lets make this easier for you: Below you will have a button to create a spawn point, simple select the team " +
                "and click on the button <b> Create Spawn Point.</b> A spawn point will be created and can now be selected in the scene view for positioning on the map.");
            Space();
        ...
        {
            DrawText("Now, in order for AI agents work on this map, you need to bake the <b>Navmesh</b>, To review, a navmesh is the area where AI Agents can move. This is calculated based on navmesh static objects" +
                " in your scene. For more info and a more detailed explanation of how to bake a Navmesh, check out this link: ");
            ...
            DrawText("You can use as many as you want, if you don't need that many, simply delete some of them. if you want more, simply duplicate them.\n" +
                "Select each individually and position in a strategic point on the map. You can preview the area enabling <b>Show Gizmos</b> in <i>AIManager -> bl_AIManager -> Show Gizmos</i>");
            DrawImage(GetServerImage(7));
        }
        else if(subStep == 5)
        {
            DrawText("Nice! Now you have your scene set up and ready. Now you need to list it in the available scene list so that players can select your scene when creating a room. " +
                " You can do it manually adding a new field in the list <b>AllScenes</b> in GameData: <b><i>(Resources folder of MFPS) GameData -> AllScenes -> Add a new field</i></b> and fill in the required info\nor you can do it automatically here," +
                " Simply set a name for the map and a sprite preview.");
