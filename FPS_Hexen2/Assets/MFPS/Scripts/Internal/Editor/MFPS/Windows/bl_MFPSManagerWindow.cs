...
public class bl_MFPSManagerWindow : EditorWindow
{
    ...
    private Color altColor = new Color(0.07f, 0.07f, 0.07f, 1.00f);
    private Color hightlightColor = new Color(0.1176471f, 0.7523355f, 1f, 1.00f);
    private Color whiteColor = new Color(0.754717f, 0.754717f, 0.754717f, 1.00f);
    ...
    public Dictionary<string, GUIStyle> styles = new Dictionary<string, GUIStyle>() { { "panelButton", null }, { "titleH2", null }, { "borders", null }, { "textC", null } };
    ...
    private Vector2 weaponScroll = Vector2.zero;
    ...
    private void OnGUI()
    {
        ...
        EditorGUI.DrawRect(new Rect(0, 0, position.width, position.height), altColor);
...
#else
        DrawDisableAddon("Input Manager", "INPUT_MANAGER");
#endif
    }
...
#region Drawers
    ...
    void DrawEditorOf(ScriptableObject scriptable)
    {
        ...
        var editor = Editor.CreateEditor(scriptable);
    ...
    {     
        ...
        if (GetAddonsInfo(addonKey).IsAddonInProject())
        {
        ...
        if (MFPSEditorStyles.ButtonOutline("Addon Manager", Color.yellow, GUILayout.Width(110)))
        {
            ...
            EditorWindow.GetWindow<MFPSAddonsWindow>().OpenAddonPage(GetAddonsInfo(addonKey).NiceName);
        }
        GUILayout.FlexibleSpace();
        EditorGUILayout.EndHorizontal();
        GUILayout.Space(5);
