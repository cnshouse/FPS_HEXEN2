...
public class bl_NetworkItem : MonoBehaviour
{
    ...
    public string ItemName
    {
        get
        {
            ...
            {
                m_itemName = $"{gameObject.name.Replace(" (Clone)", "")} [{bl_UtilityHelper.GenerateKey()}]";
            }
            return m_itemName;
    ...
    void Init()
    {
        if (isInitializated)
        {
            ...
            bl_ItemManager.Instance.PoolItem(m_itemName, this);
            return;
        }
        ...
        bl_ItemManager.Instance.PoolItem(ItemName, this);
    ...
    private void OnValidate()
    {
        ...
        if (Application.isPlaying || !isSceneItem || bl_ItemManager.Instance == null) return;
