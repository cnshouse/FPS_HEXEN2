...
using System.Linq;
using System.IO;
public class ShowPrefebMatEditor: EditorWindow {
	...
	Renderer[] mats;
	[MenuItem("ReplaceShader/Show prefeb Mater")]
	private static void showEditor()
	{
		ShowPrefebMatEditor window = (ShowPrefebMatEditor)EditorWindow.GetWindow (typeof (ShowPrefebMatEditor));
	...
	void OnGUI () {
		...
		scrollPos = GUILayout.BeginScrollView(scrollPos, true, true);
		{
			...
			if (GUILayout.Button ("show")) {
				mats = ((GameObject)selection).GetComponentsInChildren<Renderer> (true);
			}
			if (selection != null) {
				foreach (Renderer s in mats) {
					foreach (Material m in s.sharedMaterials) {
						if (m != null) {
							if(AssetImporter.GetAtPath (AssetDatabase.GetAssetPath(m.GetInstanceID())).assetBundleName =="")
	...
	public static List<T> FindAssetsByType2<T>()  where T : UnityEngine.Object
	{
		...
		for( int i = 0; i < guids.Length; i++ )
		{
			...
			T asset = AssetDatabase.LoadAssetAtPath<T>( assetPath );
			if (asset.name.Contains ("Standard")) {
				Debug.Log (assetPath);
			}
	...
	{
		foreach (Material m in AllMaterials) {
			if (m.name.Contains ("Default")) {
				Debug.Log ("m.shader.name: "+m.shader.name);
			}
